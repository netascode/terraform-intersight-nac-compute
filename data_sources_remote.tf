...
data "intersight_certificatemanagement_policy" "certificate_management_policy" {
  for_each = (var.manage_intersight_policies || !var.manage_intersight_templates) ? {} : {
    for k in local._certificate_management_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_compute_scrub_policy" "scrub_policy" {
  for_each = (var.manage_intersight_policies || !var.manage_intersight_templates) ? {} : {
    for k in local._scrub_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_storage_drive_security_policy" "drive_security_policy" {
  for_each = (var.manage_intersight_policies || !var.manage_intersight_templates) ? {} : {
    for k in local._drive_security_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_adapter_config_policy" "adapter_configuration_policy" {
  for_each = (var.manage_intersight_policies || !var.manage_intersight_templates) ? {} : {
    for k in local._adapter_configuration_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_deviceconnector_policy" "device_connector_policy" {
  for_each = (var.manage_intersight_policies || !var.manage_intersight_templates) ? {} : {
    for k in local._device_connector_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_memory_persistent_memory_policy" "persistent_memory_policy" {
  for_each = (var.manage_intersight_policies || !var.manage_intersight_templates) ? {} : {
    for k in local._persistent_memory_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_fabric_mac_sec_policy" "mac_sec_policy" {
  for_each = var.manage_intersight_policies ? {} : {
    for k in local._mac_sec_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_ippool_pool" "ip_pool" {
  for_each = (var.manage_intersight_pools || !var.manage_intersight_policies) ? {} : {
    for k in local._ip_pool_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}

data "intersight_vnic_iscsi_adapter_policy" "iscsi_adapter_policy" {
  for_each = (var.manage_intersight_policies || !var.manage_intersight_policies) ? {} : {
    for k in local._iscsi_adapter_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_vnic_iscsi_static_target_policy" "iscsi_static_target_policy" {
  for_each = var.manage_intersight_policies ? {} : {
    for k in local._iscsi_static_target_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
...
data "intersight_vnic_iscsi_boot_policy" "iscsi_boot_policy" {
  for_each = (var.manage_intersight_policies || !var.manage_intersight_templates) ? {} : {
    for k in local._iscsi_boot_policy_ref_keys : k => { name = split("/", k)[1] }
  }
  name = each.value.name
}
